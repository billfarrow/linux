...
int upc_set_qe_mux_clock(int upc_num, const char *clk, enum clk_dir mode)
{
	...
	out_be32(&qe_immr->qmx.cmxupcr,
		(in_be32(&qe_immr->qmx.cmxupcr) & ~clock_mask) |
		clock_bits);
				...
				int rpp, struct upc *upc_regs)
{
	...
	value = in_be32(reg);
	if (rpp == 1)
		value |= 1 << (UPC_SLOT_EP_MAX_NUM - 1 - ep_num);
	else if (rpp == 0)
		value &= ~(1 << (UPC_SLOT_EP_MAX_NUM - 1 - ep_num));
	out_be32(reg, value);
...
int upc_init(void)
{
	...
	for (np = NULL; (np = of_find_node_by_type(np, "upc")) != NULL; ) {
		...
		for (slot = np;
		 (slot = of_find_node_by_type(slot, "upc-slot")) != NULL; ) {
